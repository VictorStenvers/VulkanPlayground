...
// Choose the validation layers to be used
...
// Functions for creating and destroying a debug messenger
...
struct QueueFamilyIndices {
	// A simple struct for containing and using queue families
...
private:
	...
	void createInstance() {
		...
		// Setup validation layers if requested
	...
	void setupDebugMessenger() {
		// Set up the debug messenger for use with validation layers
	...
	void populateDebugMessengerInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
		// Fill in the createInfo for the debug messenger
		createInfo = {};
		createInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
		createInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
		createInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT;
		createInfo.pfnUserCallback = debugCallback;
		//createInfo.pUserData = nullptr; // Optional param
	}
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
		...
		void* pUserData) {
		// Callback function for debug mode
	...
	void pickPhysicalDevice() {
		// Choose a GPU suitable for the application from the available devices
	...
	bool isDeviceSuitable(VkPhysicalDevice device) {
		// Check if the given device includes support for all required queue families
	...
	bool checkDeviceExtensionSupport(VkPhysicalDevice device) {
		// Check if the given device includes support for the selected extensions
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		// Find if all queue families required are supported by the device
		...
		// Search through all queuefamilies for the device and update the indices accordingly
	...
	void createLogicalDevice() {
		// Create a logical device for the selected physical device for interfacing
	...
	void createSwapChain() {
		...
		// Choose the best suited parameters for the swap chain
		...
		// Build the createinfo for the swap chain
		...
		// Define how to handle different queue families if applicable
		QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
		...
		// Save the image format and extent to member variables
		swapChainImageFormat = surfaceFormat.format;
		swapChainExtent = extent;
	}

	SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device) {
		// Query for all relevant swapchain support details
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
		// Choose the best possible surface format
	...
	VkPresentModeKHR chooseSwapSurfacePresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
		// Choose the best possible present mode
	...
	VkExtent2D chooseSwapSurfaceExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
		// Choose the best possible surface extent

		// Only change the surface extent if allowed by the window manager
	...
	void createImageViews() {
		// Create image views for accessing the images in the swap chain
		...
		for (int i = 0; i < swapChainImageViews.size(); i++) {
			...
			// Define any swizzle for the colour channels
			createInfo.components.r = VK_COMPONENT_SWIZZLE_IDENTITY;
			createInfo.components.g = VK_COMPONENT_SWIZZLE_IDENTITY;
			createInfo.components.b = VK_COMPONENT_SWIZZLE_IDENTITY;
			createInfo.components.a = VK_COMPONENT_SWIZZLE_IDENTITY;

			// Define the image purpose and parts to be accessed
	...
	void createGraphicsPipeline() {
		...
		// Load all shaders and build create info
...
#else
		// Create a static viewport and scissor
		VkViewport viewport{};
		viewport.x = 0.0f;
		viewport.y = 0.0f;
	...
	VkShaderModule createShaderModule(const std::vector<char>& code) {
		// Create a shader module from the given spir-v code
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		// Set up the related renderpass info
...
#ifdef DYNAMIC_VIEWPORT
		// Create the viewport and scissor if these should be dynamic
		VkViewport viewport{};
		viewport.x = 0.0f;
		viewport.y = 0.0f;
